...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , poca_antiguedad         := ifelse( cliente_antiguedad<4, 
                                          1, 0)]
...
Lags  <- function( cols, nlag, deltas )
{
  ...
  if( deltas )
  {
    sufijodeltap  <- paste0( "_delta", nlag )

    for( vcol in cols )
    {
     dataset[,  paste0(vcol, sufijodeltap) := get( vcol)  - get(paste0( vcol, sufijo))]
    }
  }

  #agrego los deltas porcentuales de los lags, con un "for" nada elegante
  
  if( deltasp )
  {
    sufijodelta  <- paste0( "_deltapercentage", nlag )
    
    for( vcol in cols )
    {
      dataset[,  paste0(vcol, sufijodelta) := (get( vcol)  - get(paste0( vcol, sufijo))) / get(paste0( vcol, sufijo))]
    }
  }
